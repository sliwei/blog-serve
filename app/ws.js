...
function createWebsocketMiddleware(app, propertyName = 'ws', options) {
  console.log('aa')
  if (!options) options = {}
  ...
  if (parseInt(process.versions.node) < 10 && !options.noServerWorkaround) {
    ...
    if (!options.server) {
      ...
      if (!serversPatched.has(options.server)) {
        ...
        options.server.on('upgrade', (req) =>
          options.server.emit('request', req, new http.ServerResponse(req))
        )
