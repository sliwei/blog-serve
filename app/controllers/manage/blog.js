...
const edit = async (ctx, next) => {
  ...
  if (edit) {
    if (canEdit) {
      ...
      try {
        await BstuBlog.update(upDat, { where: { id: dat.id } })
    ...
    res = await BstuBlog.create(newDat, {
      include: [{ model: BstuBlogTag, as: 'blog_tag' }]
    })
    // res {xxx-xxx}
  }
...
const blog_status = async (ctx, next) => {
  ...
  let res = await BstuBlog.update(upDat, { where: { id: dat.id } })
  if (ctx.state(res)) {
    ctx.DATA.code = 0
  }
  ctx.body = ctx.DATA
    ...
    include: [
      {
        model: VBstuBlogTag,
        as: 'tag_list',
        attributes: ['name', ['t_id', 'id']]
      }
    ],
    where: {
      [Op.or]: [{ code: code }, { id: id }]
    }
  })
  ctx.body = ctx.DATA
}
